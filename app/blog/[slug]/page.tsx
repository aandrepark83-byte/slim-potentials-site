...
export const generateStaticParams = async () => {
  try {
    if (!process.env.BASEHUB_TOKEN) {
      console.warn("BASEHUB_TOKEN not found, returning empty static params")
      return []
    }
    ...
    console.warn("Failed to generate static params from BaseHub:", error)
    return []
  }
}
...
export const generateMetadata = async ({
  ...
  try {
    ...
    console.warn("Failed to generate metadata from BaseHub:", error)
    return {
      title: "Blog Post",
      description: "Blog post content",
    }
...
export default async function BlogPage({ params: _params }: { params: Promise<{ slug: string }> }) {
  try {
            ...
            <div className="flex max-w-screen-lg items-center justify-center gap-12 text-base">
              {blogpost.authors.map((author) => (
                <figure key={author._id} className="flex items-center gap-2">
                  <Avatar key={author._id} {...author.image} alt={`${author._title} - Author`} className="!size-11" />
                  {author._title}
                </figure>
              ))}
            </div>
    ...
    console.warn("Failed to load blog post from BaseHub:", error)
    return notFound()
  }
}
