...
import type { planFragment, pricingTableFragment, valueFragment } from "./fragments"
          ...
          {categories.items.map((category, i) => (
            <React.Fragment key={category._id}>
              ...
              {category.features.items.map((feature) => (
                <tr key={feature._id} className="border-b border-[--border-70] dark:border-[--dark-border-70]">
...
function TableCell<T extends React.ElementType = "td">({
  ...
  type,
  ...props
}: TableCellProps<T> & React.ComponentPropsWithoutRef<T> & VariantProps<typeof $tableCell>): React.JSX.Element {
...
function FeatureTitle(feature: PricingTableProps["categories"]["items"][0]["features"]["items"][0]) {
  return (
    <th className="w-auto">
      <TableCell align="start" as="div" type="primary">
        <p>{feature._title}</p>
        {feature.tooltip ? (
          <SimpleTooltip className="max-w-[320px]!" content={feature.tooltip} side="right" sideOffset={4}>
...
function CategoryHeader({
  ...
  return (
    <tr>
      <th className="w-auto">
        <TableCell align="start" as="div" className={clsx("px-3 pb-2 pt-10", className)} type="primary">
  ...
  return plan ? (
    <th className="w-[1fr] pb-2 pt-6">
      <span className="flex flex-col items-center gap-3 font-normal">
        ...
        <ButtonLink href="/products" intent={plan.isMostPopular ? "primary" : "secondary"}>
          Get started
        </ButtonLink>
      </span>
    </th>
        ...
        {value ? (
          value.value?.__typename === "BooleanComponent" ? (
            value.value.boolean ? (
              ...
              <span className="text-xl text-[--text-tertiary] dark:text-[--dark-text-tertiary]">&mdash;</span>
            )
