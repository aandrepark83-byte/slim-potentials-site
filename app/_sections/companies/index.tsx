...
export function Companies(props: CompaniesProps) {
  return (
    <Section container="full">
      <h2 className="text-center tracking-tight text-[--dark-text-tertiary] opacity-50">{props.subtitle}</h2>
      <div className="no-scrollbar flex max-w-full justify-center overflow-auto">
        ...
        <div className={clsx("flex shrink-0 items-center gap-4 px-6 lg:gap-6 lg:px-12", s.scrollbar)}>
          {props.companies.map((company) => (
            <figure key={company.image?.url ?? company._title} className="flex h-16 items-center px-2 py-3 lg:p-4">
              <Image
                alt={company._title}
