...
type TestimonialsSliderQuotes = Array<{
  _id: string
  quote: string
  author: {
    _title: string
...
export function Slider({
  ...
  quotes: TestimonialsSliderQuotes
...
export function VainillaCard({ quote, author }: TestimonialsSliderQuotes[0]) {
  return (
          ...
          <div className="pr-5">
            {author.company.image ? (
              <Image
                ...
                height={48}
                src={author.company.image.url || "/placeholder.svg"}
                width={48}
              />
            ) : null}
          </div>
...
export const TesimonialCard = React.memo(
  VainillaCard,
  (prevProps, nextProps) => prevProps.quote === nextProps.quote && prevProps.author === nextProps.author,
)
